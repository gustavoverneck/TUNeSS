...
!     GM3
c.....................................................................
      ! gs=dsqrt(9.927d0)/hc*m	!constante de acoplamento sigma convertida x massa proton
      ! gv=dsqrt(4.820d0)/hc*m	!constante de acoplamento omega convertida x massa proton
      ! gr=dsqrt(4.791d0)/hc*m	!constante de acoplamento rho convertida x massa proton
      ! rb=0.008659d0!*gs**3*2.d0 !0.005896d0*gs**3
      ! rc=-0.002421d0!*gs**4 !-0.006426*gs**4
      ! rxi=0.d0
      ! xs = 0.7d0
      ! xv = 0.783d0
...
!     GM1
c.....................................................................
      ! gs=dsqrt(11.785d0)/hc*m	! Normalized Scalar Meson coupling constant
      ! gv=dsqrt(7.148d0)/hc*m	! Normalized Vector Meson coupling constant
      ! gr=dsqrt(4.41d0)/hc*m	! Normalized Iso-Vector Meson coupling constant
      ! rb=0.002948d0
      ! rc=-0.001071d0
      ! rxi=0.d0
      ! xs = 0.7d0
...
c---------------------------------------------------------------------
      !bg= 1.0d17                          ! Magnetic field in Gauss
...
c---------------------------------------------------------------------
      ...
      do i=1, npoint
            ...
            if (ener .GE. 0.0d0 .and. press .GE. 0.0d0) then
                  write(4,10) nbtd/denss, ener, press, nl(1), nl(2), 
...
c	spin up
      ...
      kfnu2=efn**2-(mns)**2-2.d0*mb(1)*csi*bg-(csi*bg)**2
...
c	tratamento energi de fermi eletron
      ne=0
      do i=1,9999
         fe2=mue**2-(ml(1)**2+2*qe*b*(i-1)+2.d0*ml(1)*csi*bg
     &+(csi*bg)**2)
      ...
      enddo
 3    continue
      efe=dsqrt(fe(1)**2+ml(1)**2+2.d0*ml(1)*csi*bg+(csi*bg)**2)             !eletron fermi energy
...
c	spin up
     ...
     &-(dsqrt(mns**2+2*qe*b*(i-1)+2.d0*mb(2)*csi*bg
     &+(csi*bg)**2))**2 			!proton fermi mom squared
...
c      efp=mup-vomega-vrho/2.0d0 	!proton fermi energy
      efp=dsqrt(fpu(1)**2+(mns)**2+2.d0*mb(2)*csi*bg
     &+(csi*bg)**2) 	!proton fermi energy
...
c	spin up xxxxx acho q não incluiu o magneton de bohr aqui
     ...
     &-(dsqrt(msms**2+2*qe*b*(i-1)+2.d0*mb(4)*csi*bg
     &+(csi*bg)**2))**2 !sigma- fermi mom squared
 ...
 61    continue                  
      ...
      efsm=dsqrt(fsm(1)**2+(msms)**2+2.d0*mb(4)*csi*bg
     &+(csi*bg)**2) !sigma- fermi energy
...
c	tratamento energi de fermi sigma+
     ...
     &-(dsqrt(msps**2+2*qe*b*(i-1)+2.d0*mb(6)*csi*bg
     &+(csi*bg)**2))**2 !sigma+ fermi mom squared
 ...
 62    continue                  
      ...
      efsp=dsqrt(fsp(1)**2+(msps)**2+2.d0*mb(6)*csi*bg
     &+(csi*bg)**2) !sigma+ fermi energy
...
c	tratamento energi de fermi xi-
     ...
     &-(dsqrt(mxms**2+2*qe*b*(i-1)+2.d0*mb(7)*csi*bg
     &+(csi*bg)**2))**2 !xi- fermi mom squared
 ...
 63    continue                  
      ...
      efxm=dsqrt(fxm(1)**2+(mxms)**2+2.d0*mb(7)*csi*bg
     &+(csi*bg)**2) !sigma- fermi energy
...
c	tratamento energi de fermi proton down
     ...
     &-(dsqrt(mns**2+2*qe*b*i+2.d0*mb(2)*csi*bg
     &+(csi*bg)**2))**2 !proton fermi mom squared
...
c	tratamento energi de fermi lambda0
      rkfl02 = (mul0 - xv*vomega)**2 - ml0s**2-2.d0*mb(3)*csi*bg
     &-(csi*bg)**2             !lambda0 momento de fermi ao quadrado
       if (rkfl02 .gt. 0.d0) then                             ! condiçaõ de existência
           rkfl0 = dsqrt(rkfl02)           
           else
           rkfl0 = 0.0d0           
       endif
       efl0 = dsqrt(ml0s**2 + rkfl0**2+2.d0*mb(3)*csi*bg
     &+(csi*bg)**2)           !lambda0 fermi energy

c	===========================================
c	tratamento energi de fermi sigma0
      rkfs02 = (mus0 - xv*vomega)**2 - ms0s**2-2.d0*mb(5)*csi*bg
     &-(csi*bg)**2              !sigma0 momento de fermi ao quadrado
       if (rkfs02 .gt. 0.d0) then                             ! condiçaõ de existência
           rkfs0 = dsqrt(rkfs02)           
           else
           rkfs0 = 0.0d0           
       endif
       efs0 = dsqrt((ms0s**2 + rkfs0**2+2.d0*mb(5)*csi*bg
     &+(csi*bg)**2))           !sigma0 fermi energy

c	===========================================
c	tratamento energi de fermi xi0
      rkfx02 = (mux0 - xv*vomega -(xv*vrho/2.d0))**2 - mb(8)**2
     &-2.d0*mx0s*csi*bg-(csi*bg)**2              !xi0 momento de fermi ao quadrado
       if (rkfx02 .gt. 0.d0) then                             ! condiçaõ de existência
           rkfx0 = dsqrt(rkfx02)           
           else
           rkfx0 = 0.0d0           
       endif
       efx0 = dsqrt((mx0s**2 + rkfx0**2+2.d0*mb(8)*csi*bg
     &+(csi*bg)**2))           !xi0 fermi energy
...
c	tratamento energi de fermi muon
      ...
      nu=0
      do i=1,9999
         rkfmu2=mue**2-(ml(2)**2+2*qe*b*(i-1)+2.d0*ml(2)*csi*bg
     &+(csi*bg)**2)
      ...
      enddo
 7    continue
      efmu=dsqrt(fmu(1)**2+ml(2)**2+2.d0*ml(2)*csi*bg
     &+(csi*bg)**2)	!muon fermi energy
...
c	densidade do proton up
      rhosp=0.0d0
      do i=1,npu
      mnsu=dsqrt(mns**2+2.0d0*qe*b*(i-1)+2.d0*mb(2)*csi*bg
     &+(csi*bg)**2)
...
c	densidade do proton down
      do i=1,npd
      mnsd=dsqrt(mns**2+2.0d0*qe*b*i+2.d0*mb(2)*csi*bg
     &+(csi*bg)**2)
...
c	densidade sigma-
      rhossm=0.0d0
      do i=1,nsm
      msmsu=dsqrt(msms**2+2.0d0*qe*b*(i-1)+2.d0*mb(4)*csi*bg
     &+(csi*bg)**2)
...
c	densidade sigma+
      rhossp=0.0d0
      do i=1,nsp
      mspsu=dsqrt(msps**2+2.0d0*qe*b*(i-1)+2.d0*mb(6)*csi*bg
     &+(csi*bg)**2)
...
c	densidade xi-
      rhosxm=0.0d0
      do i=1,nxm
      mxmsu=dsqrt(mxms**2+2.0d0*qe*b*(i-1)+2.d0*mb(7)*csi*bg
     &+(csi*bg)**2)
...
c	up
      re3p=0.0d0
      do i=1,npu
      mnsu=dsqrt(mns**2+2*qe*b*(i-1)+2.d0*mb(2)*csi*bg
     &+(csi*bg)**2)
...
c	down
      do i=1,npd
      mnsd=dsqrt(mns**2+2*qe*b*i+2.d0*mb(2)*csi*bg
     &+(csi*bg)**2)+ammp*b
...
c  energia sigma-
      re3sm=0.0d0
      do i=1,nsm
         msmsu=dsqrt(msms**2+2*qe*b*(i-1)+2.d0*mb(4)*csi*bg
     &+(csi*bg)**2)
...
c  energia sigma+
      re3sp=0.0d0
      do i=1,nsp
         mspsu=dsqrt(msps**2+2*qe*b*(i-1)+2.d0*mb(6)*csi*bg
     &+(csi*bg)**2)
...
c  energia xi-
      re3xm=0.0d0
      do i=1,nxm
         mxmsu=dsqrt(mxms**2+2*qe*b*(i-1)+2.d0*mb(7)*csi*bg
     &+(csi*bg)**2)
...
c  energia eletron
      ...
      re3le=0.0d0
      do i=1,ne
         ml1=dsqrt(ml(1)**2+2*qe*b*(i-1)+2.d0*ml(1)*csi*bg
     &+(csi*bg)**2)
...
c  energia muon
      re3lmu=0.0d0
      do i=1,nu
         ml2=dsqrt(ml(2)**2+2*qe*b*(i-1)+2.d0*ml(2)*csi*bg+(csi*bg)**2)
